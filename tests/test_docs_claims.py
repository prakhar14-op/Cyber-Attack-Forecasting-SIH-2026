...
# generated, third-party node_modules and built dist are regenerable dependencies/bundles,
# and the dot-directories are `.git` (every historical revision of
...
PRUNED_DIR_NAMES = frozenset({"__pycache__", "node_modules", "dist"})
...
def test_no_exclusion_has_widened_without_being_written_down():
    ...
    assert PRUNED_DIR_NAMES == frozenset({"__pycache__", "node_modules", "dist"}), (
        "PRUNED_DIR_NAMES has changed. It prunes generated bytecode, dependencies (node_modules), "
        "and build outputs (dist), and the walk separately prunes dot-directories. Adding a real source "
        "directory here removes every file under it from the scan without excluding a single "
